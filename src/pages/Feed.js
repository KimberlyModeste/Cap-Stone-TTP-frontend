...
const Feed = ({ save }) => {
  ...
  const { loading, data: { getPosts: postsFromDB } = {} } = useQuery(
    FETCH_POSTS_QUERY
  );
  ...
  return (
    <div>
      <video
        src="https://res.cloudinary.com/dnkxmjpxy/video/upload/v1612548103/Pexels_Videos_1204911_kgmrgx.mp4"
        autoPlay
        loop
        muted
      />
      ...
      <Grid.Row className="page-title">
        <p style={{ margin: "0 0 0 0 ", fontFamily: "Impact, fantasy" }}>
          Trending
        </p>
      </Grid.Row>
      <Grid.Column columns={4}>
        {loading ? (
          <p style={{ fontSize: "20px", margin: "2rem auto 2rem auto" }}>
